...
func init() {
	...
	if os.Getenv("KOPIA_STRESS_REPO_PATH") != "" {
		fRepoDir = os.Getenv("KOPIA_STRESS_REPO_PATH")
	}

	if os.Getenv("KOPIA_STRESS_REPO_BUCKET") != "" {
		fRepoBucket = os.Getenv("KOPIA_STRESS_REPO_BUCKET")
	}
...
func BenchmarkBlockManager(b *testing.B) {
	...
	for i := 0; i < n; i++ {
		...
		if i%100 == 0 {
			RunKopiaSubcommand(b, ctx, fmt.Sprintf("maintenance.run.%%s.%%d.%d", i), tdirs, app, kpapp, "maintenance", "run",
				fmt.Sprintf("--config-file=%s", tdirs.configFilePath),
				tdirs.snapPath)
		}
